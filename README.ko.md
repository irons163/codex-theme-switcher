...
- 기본적으로 macOS 언어를 따르며 설정에서 영어, 번체 중국어, 간체 중국어, 프랑스어,
  스페인어, 일본어 또는 한국어를 직접 선택할 수도 있습니다. 자동 모드에서 지원하지 않는
  언어는 영어로 대체됩니다.
...
지원합니다. 별도 이미지를 DOM 오브 안에 원형으로 넣고 크기 모드, 초점, 불투명도,
흐림과 안쪽 여백을 개별 설정할 수도 있습니다. 이 이미지는 기본 Voice 스프라이트의
맥동을 따라갈 수 있고 강도를 0~2배로 조절할 수 있습니다. 다른 오브 효과도 독립적으로 조절됩니다.
말하는 인물에는 입이 열린 순서대로 최대 9장의 이미지를 넣을 수 있습니다. runtime은 Voice 출력
강도에 감도, 노이즈 게이트, 반응 곡선, 빠른 입 열기와 부드러운 입 닫기를 적용하고 가장 가까운
입 모양으로 바로 전환합니다. 이는 음량 기반 애니메이션이며 음소별 립싱크는
아닙니다. 한 장의 2×2 또는 3×3 이미지도 가져올 수 있으며 왼쪽에서 오른쪽, 위에서 아래 순서로
자동 분할합니다. [2×2](Examples/voice-mouth-sprites/anime-girl-mouth-2x2.png) 및
[3×3 애니메이션 캐릭터 예시](Examples/voice-mouth-sprites/anime-girl-mouth-3x3.png).
