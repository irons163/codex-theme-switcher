...
- デフォルトでは macOS の優先言語に従い、設定から英語、繁体字中国語、簡体字中国語、
  フランス語、スペイン語、日本語、韓国語を手動選択することも可能。自動モードで未対応の
  言語を使用している場合は英語へフォールバック。
...
対応します。さらに別の画像を DOM オーブ内に円形で配置し、サイズモード、焦点、
不透明度、ぼかし、内側余白を個別に設定できます。この画像はネイティブ Voice
スプライトの脈動に追従でき、強さを 0〜2 倍で調整できます。その他のオーブ効果も独立して調整
できます。話す人物には口の開き順に最大9枚の画像を設定できます。runtimeはVoiceの出力強度に
感度、ノイズゲート、レスポンス曲線、素早い開口と滑らかな閉口を適用し、最も近い口形へ
直接切り替えます。これは音量駆動であり、音素単位のリップシンクでは
ありません。1枚の2×2または3×3画像も直接読み込め、左から右、上から下の順に自動分割します。
[2×2](Examples/voice-mouth-sprites/anime-girl-mouth-2x2.png)および
[3×3のアニメ人物サンプル](Examples/voice-mouth-sprites/anime-girl-mouth-3x3.png)。
