...
独立的“圆球内部图像”，可设置尺寸模式、焦点、不透明度、模糊与内缩；圆球缩放、
不透明度、亮度、对比度、饱和度、色相旋转、模糊、光晕与背景色也可分别调整。
圆球图像支持当前的 `.codex-avatar-root` DOM 实现；如果未来改成原生或纯 Canvas 圆球，
其内部可能无法由 CSS 控制。预览依照实测的 `408:400` overlay 比例显示，但用户拖动圆球后，
runtime 位置仍可能不同。
